...
public class AssetLoader : SingletonBase<AssetLoader>
{
    ...
    {
        var bytes = await GetInstance().DownLoadByteAsync(filePah, token);
    ...
    private async UniTask<AssetLoaderResult<Texture2D>> LoadTextureLocal(string key, CancellationToken token)
    {
        var result = new AssetLoaderResult<Texture2D>(key);
        await result.LoadAsync(token);
        return result;
    }
    private async UniTask<byte[]> DownLoadByteAsync(string filePah, CancellationToken token)
    {
